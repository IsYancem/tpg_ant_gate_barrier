...
{
    public class CWeightWs : IWeightWs
    {
        ...
        public async Task HandleWebSocketConnection(WebSocket socket, int gate, string apiKey)
        {
            ...
            _log.Info($"[WeightWs] Cliente conectado | Gate:{gate}");
            ...
            {
                _log.Warn($"[WeightWs] Socket cerrado inesperadamente | Gate:{gate} | {ex.Message}");
            }
            ...
            _log.Info($"[WeightWs] Cliente desconectado | Gate:{gate}");
        }
